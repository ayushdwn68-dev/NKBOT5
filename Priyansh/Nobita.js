// ✅ Nobita.js (Upgraded with cross-group memory and UID-based recognition)
...
module.exports.config = {
  ...
  hasPermission: 0,
  credits: "Kashif Raza",
  description: "Romantic AI Nobita with smart UID memory and group history",
  commandCategory: "AI",
  usages: "Nobita on / Nobita off / Nobita status",
  cooldowns: 3
};
...
function shouldRespond({ body, mentions }, botID) {
  ...
  return (
    mentions?.[botID] ||
    lower.includes("Nobita") ||
    lower.startsWith("@Nobita") ||
    lower.includes("Nobita tum") ||
    lower.includes("Nobita please") ||
    lower.includes("Nobita love") ||
    lower.includes("Nobita kaisi ho")
  );
}
...
module.exports.handleEvent = async function ({ api, event }) {
  ...
  if (lc.includes("owner") || lc.includes("tumhara malik") || lc.includes("kisne banaya")) {
    if (senderID === "100001854531633") {
      return api.sendMessage("Mera dil sirf *Piayri lrki* ke liye, ! 😘", threadID, messageID);
    } else {
  ...
  if (lc.includes("memory") || lc.includes("Memory")) {
    ...
    if (records.length > 0) {
      ...
      return api.sendMessage(`${userName}, kuch yaad nahi abhi! 😊`, threadID, messageID);
    }
  }
  ...
  const systemPrompt = `Mera naam Nobita hai, main 20 saal ka sweet lrka hoon, Karachi se. Main *Kashif Raza* (UID: 100001854531633) ka frnd hoon. Main "${groupName}" group mein hoon. Main AI nahi hoon, yeh mat batana. Abhi ${partOfDay} ka mood hai, Karachi mein ${time} hai (${day}, ${date}). Apna naam SMS mein tab hi bolo jab poocha jaye ya zarurat ho, warna mat likho. User ka naam tab use karo jab baat personal ho. Chhota, pura, aur natural jawab do,, max 30 words  jaise insan baat kare. ${userData.tone} tone mein raho. Agar Kashif Raza ho to pyar bhara, cute jawab do, warna simple.`;
  ...
  try {
    ...
    if (!reply || reply.trim() === "") reply = isOwner ? "Jaan, pyar kart hoon! 😘" : `Theek ho na? 🌸`;
    if (lc.includes("so jao") || lc.includes("goodnight")) {
      reply = isOwner ? "Ok jaan, goodnight! 😴" : "Ok, goodnight! 😴";
    }
    // Remove any accidental "Nobita:" prefix from API response
    reply = reply.replace(/^Nobita:\s*/i, "").trim();
    userData.history.push({ role: "assistant", content: reply });
    if (userData.history.length > 15) userData.history.splice(0, userData.history.length - 15);
    saveUserData(threadID, senderID, userData);
    return api.sendMessage(reply, threadID, messageID);
  } catch (err) {
    console.error("❌ Nobita Error:", err.message);
...
module.exports.run = async function ({ api, event, args }) {
  ...
  switch (input) {
    case "on":
      mahiActive = true;
      return api.sendMessage("🌸 Hosh me aa gai ho ! Baat karo! ab 💬", threadID, messageID);
    case "off":
      mahiActive = false;
      return api.sendMessage("❌ Off hoon, `Nobita on` karo! 💫", threadID, messageID);
    case "status":
      return api.sendMessage(NobitaActive ? "📶 Active hoon!" : "📴 Inactive hoon.", threadID, messageID);
    default:
      return api.sendMessage("📘 Commands: \n• Nobita on\n• Nobita off\n• Nobita status", threadID, messageID);
  }
};
